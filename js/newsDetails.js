...
var newDetailsBox = new Vue({
	...
	mounted() {
		...
		this.$el.style.fontSize = FW * 0.07 + 'px';
		this.getId = this.getQueryVariable('id');
		this.$http.get('http://www.newxstudio.com/admin/login/getnewsbyid?id='+this.getId).then(res => {
			if(res.data.code == 0){
	...
	computed: {
		// 'myTime': function() {
		// 	if(this.news){
		// 		var time = (this.news.create_time.length>10)?this.news.create_time:this.news.create_time*1000; // 时间戳为10位需*1000，时间戳为13位的话不需乘1000
